...
const (
	e2eTargetImage = "edgectl-e2e-target" // Use the Debian image with mock opkg
...
func preTestCleanup(t *testing.T) {
	...
	output, err = cmd.CombinedOutput()
	if err != nil {
		t.Logf("Warning: Failed to list containers by image %s: %v\nOutput: %s", e2eTargetImage, err, output)
	}
...
func getOrCreateSSHKeyPair(t *testing.T) (string, string) {
	...
	for _, path := range privateKeyPaths {
		if _, err := os.Stat(path); err == nil {
			...
			output, err := cmd.CombinedOutput()
			if err != nil {
				t.Logf("Warning: Found private key at %s but failed to get public key: %v\nOutput: %s", path, err, output)
				continue
			}
...
func startContainerHelper(t *testing.T, sshPublicKey string) (string, error) {
	...
	t.Logf("Building Docker image: docker build -t %s -f ../../test/edgectl/e2e/testdata/Dockerfile --build-arg SSH_PUBLIC_KEY=... ../../test/edgectl/e2e/testdata", e2eTargetImage)
	buildCmd := exec.Command("docker", "build", "-t", e2eTargetImage, "-f", "../../test/edgectl/e2e/testdata/Dockerfile", "--build-arg", "SSH_PUBLIC_KEY="+sshPublicKey, "../../test/edgectl/e2e/testdata")
	...
	t.Logf("Running Docker container: docker run -d -p %s:22 %s", sshPort, e2eTargetImage)
	...
	for i := 0; i < 30; i++ { // Try for 30 seconds
		cmd := exec.Command("ssh", "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null",
			"-p", sshPort, "root@localhost", "echo 'SSH ready'")
		err := cmd.Run()
		if err == nil {
...
func TestInstallPackages(t *testing.T) {
	...
	// Test new install
	mockRunner.SetResponse("opkg list-installed | grep -q '^git '", "", "", errors.New("exit status 1")) // Not installed
	mockRunner.SetResponse("opkg update && opkg install git", "", "", nil)
	mockRunner.SetResponse("opkg list-installed | grep -q '^yq '", "", "", errors.New("exit status 1")) // Not installed
	...
	mockRunner.SetResponse("opkg list-installed | grep -q '^git '", "git - 2.34.1-1\n", "", nil) // Already installed
	mockRunner.SetResponse("opkg list-installed | grep -q '^yq '", "", "", errors.New("exit status 1")) // Not installed
	mockRunner.SetResponse("opkg update && opkg install yq", "", "", nil)
	...
	mockRunner = ssh.NewMockRunner() // Reset mock
	mockRunner.SetResponse("opkg list-installed | grep -q '^git '", "", "", errors.New("exit status 1")) // Not installed
	mockRunner.SetResponse("opkg update && opkg install git", "", "Error installing git\n", errors.New("exit status 1"))
